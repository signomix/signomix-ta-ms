...
public class MessagingResource {
    ...
    public String sendASimpleEmail(MessageWrapper messageWrapper) {
        return mailerService.send(messageWrapper.user.email, messageWrapper.subject, messageWrapper.message);
    }
    ...
    public String sendMessage(MessageWrapper messageWrapper) {
        ...
        ChannelConfig channelConfig = new ChannelConfig(getChannelConfig(messageWrapper.type, messageWrapper.user));
        ...
        switch (channelConfig.channelType.toUpperCase()) {
            case "SMTP":
                response = mailerService.send(channelConfig.address, messageWrapper.eui, messageWrapper.message);
                break;
            case "SMS":
            ...
            case "TELEGRAM":
                ...
                break;
            case "DISCORD":
                new DiscordService().send(channelConfig.address, new DiscordMessage("y","tets2"));
                response = "OK";
                break;
            case "WEBHOOK":
    ...
    @Produces(MediaType.TEXT_PLAIN)
    @Consumes(MediaType.APPLICATION_JSON)
    public String sendMessage(@PathParam String id, MessageWrapper messageWrapper) {
        System.out.println("ID: " + id);
        System.out.println("MESSAGE: " + messageWrapper.eui + " " + messageWrapper.message);
        return "";
    }
